...
import {
  InvoiceData,
  INDIAN_STATES,
  calcRetailItem, calcRetailTotals, formatCurrency, amountInWords,
  validateInvoiceNumber,
} from '@/lib/logic/invoice'

export default function LetterheadGSTPreview({ data }: { data: InvoiceData }) {
  ...
  // Item name column fills remaining width after all other fixed + optional cols
  const optColCount   = (hasDisc ? 1 : 0) + (hasCD ? 1 : 0) + (hasCess ? 1 : 0)
  const itemNameWidth = `${35 - optColCount * 5}%`
    ...
    <div id="invoice-preview" className="bg-white text-slate-800 border-2 border-slate-900 overflow-hidden font-sans text-xs shadow-sm max-w-[210mm] mx-auto">
      ...
      <div className="border-b-2 border-slate-900">
        {data.letterheadImage ? (
          <img src={data.letterheadImage} alt="Letterhead" className="w-full object-contain max-h-24" />
        ) : (
          <div className="h-16 flex items-center justify-center m-2 rounded border-2 border-dashed border-slate-200">
            <span className="text-slate-300 text-[10px] font-medium tracking-widest uppercase">Letterhead Area</span>
          </div>
        )}
        <div className="border-t-2 border-slate-900 p-1.5 text-center">
          <p className="text-xs font-bold tracking-widest text-slate-900 uppercase">{data.isProforma ? 'Proforma Invoice' : 'Tax Invoice'}</p>
        </div>
      </div>
      ...
      {/* ── ITEMS TABLE (conditional Disc%, CD%, Cess%) ── */}
      <div className="border-b-2 border-slate-900">
        <table className="w-full border-collapse text-left table-fixed">
          <thead>
            <tr className="bg-slate-50 border-b-2 border-slate-900 text-[9px] font-bold text-slate-900 tracking-tight">
              ...
              <th className="p-1 border-r border-slate-900" style={{ width: itemNameWidth }}>Item Name</th>
              ...
              {hasDisc && <th className="p-1 text-right border-r border-slate-900 w-[5%]">Disc %</th>}
              {hasCD   && <th className="p-1 text-right border-r border-slate-900 w-[5%]">CD %</th>}
          ...
          <tbody>
            {data.items.map((item, idx) => {
              ...
              return (
                <tr key={item.id} className="text-slate-800 text-[10px] tracking-tighter align-top">
                  ...
                  <td className="py-1 px-0.5 text-center font-mono border-r border-slate-900">{item.quantity} PCS</td>
                  <td className="py-1 px-0.5 text-right font-mono border-r border-slate-900">{item.mrp.toFixed(2)}</td>
                  <td className="py-1 px-0.5 text-right font-mono border-r border-slate-900">{c.effectiveRate.toFixed(2)}</td>
                  {hasDisc && <td className="py-1 px-0.5 text-right font-mono border-r border-slate-900 whitespace-nowrap">{item.discRate.toFixed(2)}%</td>}
                  {hasCD   && <td className="py-1 px-0.5 text-right font-mono border-r border-slate-900 whitespace-nowrap">{item.cdRate.toFixed(2)}%</td>}
                  ...
                  <td className="py-1 px-0.5 text-right font-mono border-r border-slate-900">{c.gstAmt.toFixed(2)}</td>
                  <td className="py-1 px-0.5 text-right font-mono font-medium text-slate-900">{c.subtotal.toFixed(2)}</td>
                </tr>
              )
            })}
            ...
            <tr className="h-20 align-top select-none pointer-events-none text-[0px]">
              ...
              <td className="border-r border-slate-900">&nbsp;</td>
              <td className="border-r border-slate-900">&nbsp;</td>
              {hasDisc && <td className="border-r border-slate-900">&nbsp;</td>}
              {hasCD   && <td className="border-r border-slate-900">&nbsp;</td>}
              <td className="border-r border-slate-900">&nbsp;</td>
            ...
            <tr className="bg-slate-50/60 font-bold text-[10px] border-t-2 border-slate-900 align-middle tracking-tighter">
              ...
              <td className="p-1 border-r border-slate-900" />
              <td className="p-1 border-r border-slate-900" />
              {hasDisc && <td className="p-1 text-right font-mono border-r border-slate-900">{totals.totalDisc.toFixed(2)}</td>}
              {hasCD   && <td className="p-1 text-right font-mono border-r border-slate-900">{totals.totalCD.toFixed(2)}</td>}
              <td className="p-1 border-r border-slate-900" />
