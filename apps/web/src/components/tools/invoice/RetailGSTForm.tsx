...
import {
  ...
  calcRetailItem, formatCurrency, computeRetailEffectiveRate,
  validateInvoiceNumber, validatePAN,
} from '@/lib/logic/invoice'

// ─────────────────────────────────────────────────────────────────────────────
// SHARED HELPERS (same as ClassicGSTForm — keep local to avoid cross-imports)
// ─────────────────────────────────────────────────────────────────────────────
...
function validatePercent(val: number, label: string): string | null {
  if (val < 0 || val > 100) return `${label} must be 0–100`
  return null
}
...
// ─────────────────────────────────────────────────────────────────────────────
// RETAIL ITEM ROW — MRP → Disc% → CD% → Effective Rate → Taxable Value
// ─────────────────────────────────────────────────────────────────────────────
function RetailItemRow({
  ...
  const [mrpExceeded, setMrpExceeded] = useState(false)
  ...
  const errMRP    = t('mrp')
    ? item.mrp <= 0
      ? 'MRP must be > 0'
      : item.mrp > 10000000
        ? 'MRP cannot exceed ₹1 Crore'
        : null
    : null
  const errDisc   = t('discRate') ? validatePercent(item.discRate, 'Trade Discount') : null
  const errCD     = t('cdRate')   ? validatePercent(item.cdRate, 'Cash Discount')     : null
  const errDiscCD = (t('discRate') || t('cdRate')) && (item.discRate + item.cdRate > 100)
    ? 'Trade Disc + CD cannot exceed 100%'
    : null
    ...
    <div className="grid gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="relative">
          ...
          <Input label="Qty *" type="number" min="1" max="1000000" step="1" value={item.quantity}
            onChange={e => {
        ...
        <div className="relative">
          ...
          <Input label="MRP (per unit) *" type="number" min="0" max="10000000" step="0.01"
            value={item.mrp}
            onChange={e => {
              const v = parseFloat(e.target.value)
              if (!isNaN(v) && v > 10000000) { setMrpExceeded(true); return }
              setMrpExceeded(false)
              onChange(item.id, 'mrp', v || 0)
            }}
            onBlur={() => touch('mrp')}
            className={mrpExceeded || errMRP ? 'border-red-400' : ''} />
          <FieldError msg={!mrpExceeded ? errMRP : null} />
        </div>
      </div>

      {/* Disc% + CD% + Computed effective rate */}
      <div className="grid grid-cols-3 gap-2">
        <div>
          <Input label="Trade Disc %" type="number" min="0" max="100" step="0.01"
            value={item.discRate}
        ...
        <div>
          <Input label="Cash Disc (CD) %" type="number" min="0" max="100" step="0.01"
            value={item.cdRate}
        ...
        <div className="flex flex-col justify-end">
          <label className="text-[10px] text-slate-500 font-medium mb-1">Eff. Rate / unit</label>
          <div className="h-9 flex items-center px-2 bg-amber-50 border border-amber-200 rounded text-xs font-mono text-amber-800 select-none">
            {formatCurrency(effectiveRate, currency as any)}
          </div>
        </div>
      </div>

      {/* Taxable subtotal pill */}
      {item.mrp > 0 && (
        <div className="flex flex-wrap gap-3 text-[10px] bg-white border border-slate-200 rounded-lg px-2.5 py-1.5">
          <span className="text-slate-500">Taxable: <span className="font-mono font-semibold text-slate-700">{formatCurrency(c.subtotal, currency as any)}</span></span>
          {item.discRate > 0 && <span className="text-slate-400">Trade Disc: <span className="font-mono line-through text-red-400">-{formatCurrency(c.discAmt, currency as any)}</span></span>}
          {item.cdRate > 0 && <span className="text-slate-400">CD: <span className="font-mono line-through text-red-400">-{formatCurrency(c.cdAmt, currency as any)}</span></span>}
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="flex flex-col justify-end">
          {customCess ? (
            <div className="relative">
              <Input label="Cess %" type="number" min="0" max="100" step="0.1"
                value={item.cessRate}
      ...
      <div className="flex justify-end gap-4 text-[10px] text-slate-500 border-t border-slate-200 pt-2 mt-1">
        ...
        {item.cessRate > 0 && (
          <span>Cess: <span className="font-mono text-slate-700">{formatCurrency(c.cess, currency as any)}</span></span>
        )}
        <span className="font-bold text-slate-800">
          Total: <span className="font-mono">{formatCurrency(c.total, currency as any)}</span>
        </span>
      </div>
    </div>
  )
...
// ─────────────────────────────────────────────────────────────────────────────
// MAIN FORM — Retail GST (MRP-based pricing with trade & cash discounts)
// ─────────────────────────────────────────────────────────────────────────────
export default function RetailGSTForm({ data, set, updateItem, addItem, removeItem }: RetailGSTFormProps) {
  const [touched, setTouched] = useState<Record<string, boolean>>({})
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── SECTION 2: SELLER ── */}
      <Section title="2. Seller / Distributor" hasError={sec2Err}>
        {/* Logo upload */}
        <div>
        ...
        <div>
          <Input label="Business / Firm Name *" value={data.sellerName}
        ...
        <Input label="Landmark" value={data.sellerLandmark || ''}
      ...
      <Section title="3. Retailer / Ship To" hasError={sec3Err}>
        <div className="bg-slate-50 p-2 rounded-lg space-y-2 border border-slate-200">
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Bill To (Retailer)</p>
          <div>
            <Input label="Retailer / Buyer Name *" value={data.buyerName}
        ...
        <div className="bg-slate-50 p-2 rounded-lg space-y-2 border border-slate-200">
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Ship To <span className="normal-case font-normal">(optional)</span></p>
          <Input label="Delivery Point Name"
            placeholder="Leave empty if same as billing"
            value={data.shipToName || ''}
            onChange={e => set('shipToName', e.target.value)} />
          <Textarea label="Delivery Address" rows={2}
            value={data.shipToAddress || ''}
            onChange={e => set('shipToAddress', e.target.value)} />
          <Input label="Delivery Landmark"
            value={data.shipToLandmark || ''}
            onChange={e => set('shipToLandmark', e.target.value)} />
          <div>
            <Input label="Ship-To GSTIN" value={data.shipToGST || ''}
      ...
      {/* ── SECTION 4: LINE ITEMS (RETAIL) ── */}
      <Section title="4. Products (MRP-based Pricing)">
        {/* Pricing model hint */}
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-[10px] text-amber-800 leading-relaxed">
          <span className="font-bold">Retail pricing chain:</span> MRP → Trade Discount% → Cash Discount% → Effective Rate → Taxable Value (qty × eff. rate)
        </div>
        ...
        {data.items.length === 0 && (
          <p className="text-[10px] text-red-500 flex items-center gap-1">
            <AlertCircle size={10} /> At least one product is required
          </p>
        )}
      ...
      <Section title="6. Notes & Terms" defaultOpen={false}>
        <Textarea label="Notes (internal / optional)" rows={2}
          ...
          placeholder="e.g. Batch no., expiry, vehicle no." />
